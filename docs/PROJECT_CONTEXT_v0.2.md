...
### 2.2 数据来源与采集参数差异（当前批次）

当前项目数据来自两类主要来源：

- **来源 A（本地 raw）**：`data/00_raw/20241206_s1bf/` 与 `data/00_raw/20241206_pig_hippocampus/` 中拆分的 OME‑TIFF zstacks（例如 S1BF、pig_hippocampus）。
- **来源 B（snoutbox）**：`/dfs/snout/Histology/RND2412/Microscopy data/Confocal scans/202512_8rats_3ROIs/` 中的 ROI 文件夹，每个 `.ome.tif` 为一个 zstack。

上述 6 个候选 zstack 的元信息特征：

- **XY 分辨率一致**：dx=dy=0.439294 µm/px（TIFF X/YResolution 与 OME‑XML 一致）。
- **位深一致**：uint8。
- **SizeX/SizeY 一致**：512×512。
- **Z 步长分两档**：
  - 来源 A：dz≈0.396329 µm。
  - 来源 B：dz≈0.201478 µm（Z 采样更密）。
- **SizeZ 不同**：约 50–73 张切片范围。
- 个别文件缺失 OME‑XML（例如 `2504_42_L_M1_S28.ome.tif`），需用同 ROI 其它 zstack 元信息作为 **dz 参考**。

### 2.3 Z 步长统一策略（推理/比较用）

为保证跨来源、跨模型的可比性，推理统一采用：

- **目标步长**：dz = 0.396 µm。
- **方法**：
  - 图像在 **Z 轴线性插值**重采样（XY 不变）。
  - 仅用于推理输入，不改变原始归档数据。
  - 记录 **原始 dz、目标 dz、重采样比例**到 sidecar 元数据（JSON）。
- **缺失 dz 情况**：从同一 ROI/同批次 zstack 估算 dz，并在元数据中标注来源与推断方式。
...
### 2.6 推理输出归档与可追溯性（新增要求）

为保证后续评估与复现实验的一致性，推理完成后需将逐 slice 预测 **重建为 zstack OME‑TIFF**，并写入可追溯元信息：

- **原始数据元信息**：保留原始 OME‑XML（尺寸、分辨率、物镜/采集参数）。
- **预测元信息**：模型名、trainer/plans、推理设置（2D/2.5D、通道数、重采样 dz、滑窗策略等）、commit/seed。
- **运行摘要**：预测日期、输入来源、样本数、异常/跳过清单。

若 OME‑TIFF 无法写入自定义扩展字段，则必须生成同名 **sidecar JSON** 记录上述信息。

### 2.7 推理前一致性检查（强制）

在任何推理任务开始前，必须做一次输入一致性检查：

- **dz/分辨率一致性**：与训练数据不一致则先重采样，记录原始 dz 与目标 dz。
- **通道数与命名**：保证与模型输入一致（例如 7ch = _0000…_0006）。
- **文件后缀**：与 dataset.json 保持一致。
...
- 银染点状噪声：背景中散落的细小高亮颗粒，不附着在髓鞘枝干上；常为“几个到几十个像素”大小的亮团，强度较高，易被误判为短线段或节点。
- 片状噪声：靠近组织 surface 的方向上，点状颗粒密度高到近似连成片；是否出现以及出现强度依赖染色质量与层面位置（有的 slice 没有，有的明显）。
- 散射噪声：confocal 成像远离组织时出现同心环式分布的随机噪音（环状纹理），可能造成伪阳性与错误连接。
...
- 固定划分标准（当前项目）：
  - 采用按 stack 分组的 **4-fold（leave-one-stack-out）** 划分，作为后续 nnUNet 训练基准。
  - 划分使用固定随机种子（seed=42），分组与划分文件长期保存。
  - 所有可比性实验必须使用同一 split；如需更改必须提前说明并记录到 Lab Report。
...
风险：训练进行中若重跑计划/预处理（或改动共享目录），可能导致预处理文件缺失/被覆盖，引发 dataloader 报错并中断训练。

缓解：训练期间避免触碰正在使用的 `nnUNet_preprocessed` 目录；如需尝试新 planner/预处理参数，必须隔离出**独立的预处理目录**（例如在方法目录下自建 `nnUNet_preprocessed`），并在 Lab Report 中记录隔离路径与原因。
